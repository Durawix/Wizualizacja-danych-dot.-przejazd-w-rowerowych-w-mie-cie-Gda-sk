...
1. Najbardziej ruchliwe stacje (>5mln) to "Pas nadmorski" oraz "al. Zwycięstwa", stacje o wysokiej liczbie przejazdów (1,5-5mln) to "ul. 3 Maja", "al. Grundwaldzka (Wrzeszcz)", "al. Grunwaldzka (UG)", "Błędnik", "al. Hallera", "ul. Kartuska", "ul. Chłopska", "Kanał Raduni". Pozostaje stacje to stacje o umiarkowanej liczbie przejazdów (<1mln)
...
przejazdy %>%
  ...
  summarise(SumaPrzejazdow = sum(Licznik, na.rm = TRUE)) %>%
  arrange(desc(SumaPrzejazdow)) %>%
  ggplot(aes(x = reorder(Stacja, SumaPrzejazdow), y = SumaPrzejazdow / 1000)) +  
  geom_col(fill = "darkorange") +
  geom_text(aes(label = sprintf("%.0fK", SumaPrzejazdow / 1000)), hjust = -0.1, size = 3) +  
  coord_flip() +
  labs(title = "Suma przejazdów rowerowych w poszczególnych stacjach",
       x = "Stacja", y = "Suma przejazdów (w tysiącach)") +
  theme_minimal() +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1))) + 
  theme(axis.text.y = element_text(size = 8, hjust = 1, angle = 0))  
```
